...
export default function Process() {
  ...
  const [failedImages, setFailedImages] = useState<boolean[]>(() => PROCESS_STEPS.map(() => false));
  ...
  return (
    <section ref={sectionRef} id="process" className="relative w-full overflow-hidden border-y border-[var(--border-subtle)] bg-[var(--background)] py-28 md:py-36 transition-colors duration-500">
      ...
      <motion.div style={{ opacity: exitOpacity }} className="pointer-events-none absolute inset-0 overflow-hidden z-0">
        <div 
          className="absolute top-0 right-0 h-[2px] w-[100px] bg-gradient-to-r from-transparent via-[var(--color-cyan)] to-white opacity-0"
          style={{
            animation: "shooting-star 12s infinite ease-in-out",
          }}
        />
        <div 
          className="absolute top-[10vh] right-[25vw] h-[1.5px] w-[80px] bg-gradient-to-r from-transparent via-[var(--color-pink)] to-white opacity-0"
          style={{
            animation: "shooting-star-fast 8s infinite ease-in-out",
            animationDelay: "4s",
          }}
        />
      </motion.div>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6">
        ...
        <div className="relative" ref={trackRef}>
          <div aria-hidden="true" className="absolute bottom-0 left-[7px] top-0 hidden w-px bg-[var(--border-subtle)] md:block" />
          ...
          {trackHeight > 0 && (
            <motion.div
              ...
              className="absolute top-0 z-20 hidden h-[22px] w-[22px] cursor-grab items-center justify-center rounded-full border-2 border-[var(--color-cyan)] bg-[var(--background)] shadow-[0_0_22px_rgba(103,232,249,0.9)] active:cursor-grabbing md:flex"
            >
          ...
          {PROCESS_STEPS.map(([number, title, description], index) => {
            ...
            return (
              ...
              >
                <span className="relative text-sm font-medium text-[var(--color-cyan)]"><span className={`absolute -left-[42px] top-1 hidden h-[14px] w-[14px] rounded-full border-2 bg-[var(--background)] transition-all duration-500 md:block ${isActive ? "border-[var(--color-cyan)] shadow-[0_0_16px_rgba(103,232,249,0.8)] scale-125" : "border-[var(--color-cyan)]/40"}`} />{number}</span>
                <div className="grid gap-5 md:grid-cols-[1fr_170px] md:items-center">
                  <div>
                    <h3 className={`text-xl font-medium md:text-2xl transition-colors duration-500 ${isActive ? "text-[var(--foreground)]" : "text-[var(--text-secondary)]"}`}>{title}</h3>
                    <p className="mt-3 max-w-2xl text-sm leading-relaxed text-[var(--text-secondary)] md:text-base">{description}</p>
                  </div>
                  <div
                    className={`relative hidden aspect-[16/10] overflow-hidden rounded-xl border md:block transition-all duration-500 ${isActive ? "border-[var(--color-cyan)]/50 scale-[1.03]" : "border-[var(--border-subtle)]"}`}
                    style={{ transform: isActive ? `rotate(${index % 2 === 0 ? -2.5 : 2.5}deg)` : "rotate(0deg)" }}
                  >
                    {failedImages[index] ? (
                      <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-[var(--color-accent)]/20 to-[var(--color-cyan)]/10 text-xs text-neutral-500">{number}</div>
                    ) : (
                      <img
